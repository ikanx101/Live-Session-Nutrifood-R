...
library(e1071)
library(pROC)
modelBayes = naiveBayes(as.factor(train_df$target)~.,data=train_df)
trainPred = predict(modelBayes, newdata = train_df, type = "class")
testPred = predict(modelBayes, newdata = test_df, type = "class")
roc_obj = roc(as.numeric(train_label_raw), as.numeric(trainPred))
auc(roc_obj)
roc_obj = roc(as.numeric(test_label_raw), as.numeric(testPred))
auc(roc_obj)


pred = predict(modelBayes, newdata = submit, type = "class")
submission = data.frame(
  PassengerId = test_submit$passenger_id,
  Survived = pred
)
write.csv(submission,"submit naive.csv",row.names = F)