...
```{r,fig.align='center',fig.retina=7}
rm(list=ls())
df = read.csv("/cloud/project/LEFO Market Research/LEFO MR 2023/Unsupervised/hierarchical/donat.csv") %>% 
     janitor::clean_names() %>%
     select(x_2,y) %>%
     rename(x = x_2)
...
```{r,fig.align='center',fig.retina=7}
rm(list=ls())
df = read.csv("/cloud/project/LEFO Market Research/LEFO MR 2023/Unsupervised/hierarchical/hi.csv") %>% 
     janitor::clean_names() %>%
     select(x_2,y) %>%
     rename(x = x_2)
