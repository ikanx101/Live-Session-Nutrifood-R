...
# oke, sekarang kita hitung utk semuanya
train = mclapply(list_teks,buat_dataset_train,
                 mc.cores = n_core)

# kita intip hasilnya
train[[2]]
# pastikan sama dengan hasil baris sebelumnya

# sekarang kita transpose datasetnya
dataset = mclapply(train,transpose,
                   mc.cores = n_core)

# kita intip lagi hasilnya
dataset[[2]]
# ==============================================================================
...
vectors = vector("list",length(dataset))
for(i in 1:length(dataset)){
  print(i)
  vectors[[i]] = vectorize(dataset[[i]])
}



# kita hitung
vectors = mclapply(dataset,vectorize,
                   mc.cores = n_core)
