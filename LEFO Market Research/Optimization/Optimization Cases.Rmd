...
Pak Agung adalah seorang pemilik kios yang menjual pupuk organik dalam kemasan karung. Selama ini, Pak Agung selalu memesan  sejumlah karung pupuk organik dari produsennya setiap hari minggu. Dia juga mencatat berapa karung pupuk yang terjual setiap  minggunya. Setiap minggunya, pupuk yang terjual tidak selalu sama, tetapi dia dapat menghitung rata-rata pupuk yang terjual setiap minggunya.

Sekarang ini, usahanya sudah berkembang pesat. Kadangkala persediaan pupuk sudah habis ketika ada orang yang akan membeli pupuk. Tetapi kadang pula sesudah satu minggu masih tersedia pupuk yang belum terjual dan ini memerlukan biaya penyimpanan bertambah. Karena itu,   dia ingin mengetahui berapa botol pupuk yang harus dipesan setiap kali memesan dan apakah dia harus memesan lebih sering atau lebih jarang.
...
1. $C_O$ adalah biaya pemesanan dalm satu kali pesan, tidak bergantung pada banyaknya pupuk yang dipesan,
1. $D$ adalah rata-rata banyaknya pupuk organik yang terjual perminggunya (satuannya `karung/7 hari`),
1. $S$ adalah biaya penyimpanan pupuk per karung per hari.

Kita asumsikan semua nilainya tetap.
...
- $O$ adalah banyaknya karung pupuk organik yang dipesan dalam satu kali pesan.
- $P$ adalah panjangnya siklus antara dua kali pemesanan (dalam hari).

### Masalah Optimisasi

Kita dapat memodelkan masalah optimisasinya dengan terlebih dahulu membuat simplifikasi dinamika inventori pupuknya sebagai berikut. Misalkan pupuk dipesan sebanyak 10  karung tiap minggu, dengan rata-rata banyaknya pupuk yang terjual adalah 10/7 karung per minggu.  Asumsikan lead time, yaitu periode  waktu yang dibutuhkan mulai dari memesan sampai barang datang, adalah 0. Maka ketika pupuk dipesan tingkat inventori adalah 0, dan sesaat setelah memesan tingkat inventory langsung berubah menjadi 10, seperti digambarkan di gambar berikut ini.

```{r out.width="70%", echo=FALSE, fig.align='center'}
knitr::include_graphics("EOQ.png")
```

Tujuan kita adalah menentukan berapa nilai $O$ dan berapa nilai $P$ agar rata-rata biaya inventory minimum. Agar lebih mudah dalam penurunan fungsi objektif, kita akan gunakan rata-rata biaya inventori per hari. Kita sebenarnya dapat juga menurunkan fungsi objektif berupa rata-rata biaya inventori per minggu atau per bulan, dan hal ini Kita ingat bahwa di masalah ini biaya pemesanan adalah tetap, tidak tergantung pada berapa banyak pupuk yang kita pesan. Maka jika kita memesan tiap $P$ hari,
...
Fungsi di atas adalah fungsi dua peubah, yang mana di kasus kita kita dapat membuatnya menjadi fungsi satu peubah dengan mensubtitusi variabel $P$ dalam variabel $O$ jika kita gunakan asumsi tambahan bahwa kita memesan di saat tingkat inventori sama dengan nol. Artinya, gunakan ilustrasi di gambar di atas pada kondisi umum yaitu kemiringan grafiknya adalah $-\frac{D}{7}$,
...
# __Regresi Linear Sebagai Masalah__ ___Optimization___
