...
Ada banyak cara dalam mengolah data di __R__. Teman-teman bisa menggunakan prinsip `tidyverse`, `data.table`, dan lainnya. Kali ini saya akan memodifikasi materi training `Sigit` dengan menggunakan `%>%` sehingga memberikan gambaran bagaimana _pipe_ __sangat amat powerful__ digunakan untuk memanipulasi _rows_ dan _columns_ dari suatu data. 
...
nomnoml::nomnoml("#direction: down,
                 ...
                 [Bebersih Data|
                    [Tipe Data] -> [Ubah ke tanggal]
                    [Tipe Data] -> [Ubah ke factor]
                    [Tipe Data] -> [Ubah ke numerik]
                    ]
...
setwd("~/Documents/Live-Session-Nutrifood-R/Materinya Sigit")
...
Kalau teman-teman menemukan pesan _error_ pada _chunk_ ini, bisa jadi karena ada satu atau lebih _library_ yang belum ter-_install_. Jadi pastikan semua sudah ter-_install_ dulu! 
...
Ingat kembali bahwa _file_ dataset __sudah ada di WORKING DIRECTORY__. 

__Ingat yah!__

Banyak _trainees_ yang sering _error_ sejak awal ini soalnya.
...
Mendapatkan _insights_ menggunakan visualisasi `ggplot`:
...
Ada `2` cara mengkonversi data tipe `character` menjadi bentuk waktu (tanggal dan jam), yakni dengan:
...
Tulisan textnya berantakan yah gaes... kita rapihkan dengan beberapa _custom setting_ sbb:
...
Sekarang kita akan melakukan _dive deeper_ dengan menganalisa _Pre-Campaign Analysis_ pada kategori: 

- Entertainment, 
- Music 
- Gaming

Kita akan lakukan _pre-processing_ dengan `dplyr` sebagai berikut:
...
refinery %>% 
  ggplot(aes(x = likesratio,
             y = dislikeratio)) +
  geom_point(aes(size = commentratio))
```
...
plot = 
  ...
  ggplot(aes(x = channel_title, y= value))+
  geom_col(aes(fill = name), position = "dodge")+
  geom_text(aes(label = label_new), size = 2.1, color = "steelblue")+ # add enhancement on geom_text()
  coord_flip()+
  labs(title = "Likeability Analysis on Popular Channels",
       x = NULL,
       y = NULL,
...
1. Tim marketing Algoritma berencana untuk melakukan campaign (iklan) pada 5 channel youtube yang memiliki likes ratio tertinggi. Karena Algoritma bergerak di bidang edukasi dan data science maka, hanya channel-channel yang memiliki video dengan kategori `Education` dan `Science and Technology` yang akan dipilih untuk melakukan campaign. Buatlah sebuah visualisasi yang menggambarkan informasi tersebut.
...
vids1 = 
  ...
  mutate(likesr = likes/views) %>% 
  group_by(channel_title, category_id) %>% 
  summarise(mean.likesr = round(mean(likesr)*100, 2)) %>% 
  ungroup() %>% 
  arrange(desc(mean.likesr)) %>% 
  head(5) 

vids1 %>% 
  ggplot(aes(x = reorder(channel_title,-mean.likesr),
             y = mean.likesr)) +
  geom_col(aes(fill = category_id)) +
  geom_text(aes(label = mean.likesr)) +
  theme_economist() +
  labs(title = "Lebih baik kita beriklan di mana?",
       subtitle = "Channel Title dari Kategori yang Diinginkan",
       x = "Channel Title",
       y = "Rata-rata Likes Ratio",
       fill = "Kategori",
       caption = "Training R\n@nutrifood") +
...
plot1 =
  ...
  ggplot(aes(x = reorder(channel_title, mean.likesr), 
             y = mean.likesr)) +
  geom_col(aes(fill = mean.likesr), show.legend = F) +
  labs(title = "Top 5 Channel based on Likes Ratio",
...
vids1 = 
  ...
  arrange(desc(mean.likesr)) %>% 
  head(5) %>% 
  mutate(text = paste0("Category: ",category_id,"\nLikes Ratio: ",mean.likesr,"%")) # penambahan \n berfungsi agar nanti saat dirender tulisan akan turun ke bawah
...
plot2 =
  vids2 %>% 
  ggplot(aes(x = trending_date,
             y = views_n)) +
  geom_point(aes(color = category_id, text = text), show.legend = F) +
  geom_line(aes(color = category_id))+
  labs(title = "Trend Total Video Trending Based on Category",
       y = NULL,
       x = "Category")
